...
public class HistoryJComboBox extends JComboBox
{
     ...
     *            Preference name to store history
     */
    public HistoryJComboBox(String pn)
    {
        this(null, pn, new Object[0]);
    }
     ...
     *            Array to start with
     */
    public HistoryJComboBox(String pn, Object[] start_array)
    {
    ...
    {
        this(transferHandler, pn, new Object[0]);
    }

    public HistoryJComboBox(TransferHandler transferHandler, String pn,
            Object[] start_array)
    {
        super(start_array);
        init(pn);
    ...
    private void culPrefs()
    {
        ...
        for (Iterator<HistoryJComboBoxLife> it = tp.iterator() ; it.hasNext()
                && cul > 0 ; --cul)
        {
     ...
     * 
     * @param item
     *            Item to be added. If null, then current is used
     */
    public void savePrefs(String item)
    {
        ...
        if (p_items.length() >= Preferences.MAX_VALUE_LENGTH)
        {
            ...
            System.out.println("HistoryJCombo:" + preferenceName + " "
                    + p_items.length() + " >= " + Preferences.MAX_VALUE_LENGTH);
            culPrefs();
     ...
     * 
     * @param num
     *            Max number of history entries, 0 mean unlimited
     */
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if (clearHistory.equals(selected))
        {
            if (JOptionPane.showConfirmDialog(this,
                    "Are you sure you want to clear history?", "Clear History",
        ...
        {
            if (JOptionPane.showConfirmDialog(this,
                    "Are you sure you want to cul history?", "Cul History",
                    JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION)
            {
                culPrefs();
                return;
...
class HistoryJComboBoxLife implements Comparable<HistoryJComboBoxLife>
{
    ...
    public HistoryJComboBoxLife(String txt)
    {
        ...
        else
        {
            try
            {
                age = new Long(items[1]).longValue();
            }
