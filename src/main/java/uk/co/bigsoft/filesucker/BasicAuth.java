...
public class BasicAuth {
	...
	// conversion table
	private static byte[] cvtTable = { (byte) 'A', (byte) 'B', (byte) 'C', (byte) 'D', (byte) 'E', (byte) 'F',
	...
	/**
	 * Encode a name/password pair appropriate to use in an HTTP header for Basic
	 * Authentication. name the user's name passwd the user's password returns
	 * String the base64 encoded name:password
	 */
	public static String encode(String name, String passwd) {
		...
		/**
		 * Loop through input with 3-byte stride. For each 'chunk' of 3-bytes, create a
		 * 24-bit value, then extract four 6-bit indices. Use these indices to extract
		 * the base-64 encoding for this 6-bit 'character'
		 */
